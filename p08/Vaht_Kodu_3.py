...
def main():
    ...
    kitarr = np.convolve(kitarr, ribafilter)
    win.nextRow()
    p2 = win.addPlot()
    tulemus = filter_in_frequecy(kitarr)
    print(kitarr)
    p2.plot(np.abs(tulemus), x=array[0:110251])

    ribafilter2 = create_bandstop(0.317460, 0.340136, 0.001)
    win.nextRow()
    filter2 = win.addPlot()
    filter2.addLegend()
    riba2 = filter_in_frequecy(ribafilter2)
    plot_filter_freq(filter2, riba2)

    tulemus = overlap_add(kitarr, ribafilter2)
    win.nextRow()
    p2 = win.addPlot()
    kitarr = filter_in_frequecy(tulemus)
    p2.plot(np.abs(kitarr), x=array[0:112251])
